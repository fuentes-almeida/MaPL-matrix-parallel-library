...
double splineC1(double x)
{
    double **curve;
    if (x<xmid1){curve=curve1;}
    else if(x<xmid2){curve=curve2;}
    else {curve=curve3;}
    int k=0;
    ...
    double s=curve[1][k-1]+curve[4][k-1]*(x-curve[0][k-1])+curve[5][k-1]*(x-curve[0][k-1])*(x-curve[0][k-1]);
    return s;
}
...
static gboolean draw_cb(GtkWidget *widget, cairo_t *cr, gpointer par)
	...
	for(k=0;k<length;k++)
        ...
        yvalue1[k]=splineC1(xvalue[k])*yscale+yoffset;
        }
...
{
    scanf("%d",&n1);
    curve1=RequestMatrixDoubleMem(6,n1);
    for (int i=0;i<n1;i++)
        scanf("%lf %lf",&curve1[0][i],&curve1[1][i]);
        curve1[4][0]=(curve1[1][1]-curve1[1][0])/(curve1[0][1]-curve1[0][0]);

    for (int i=0;i<n1-1;i++){
        curve1[2][i]=curve1[0][i+1]-curve1[0][i];
        curve1[3][i]=curve1[1][i+1]-curve1[1][i];
        curve1[4][i+1]=2*(curve1[3][i]/curve1[2][i])-curve1[4][i];
        curve1[5][i]=((curve1[3][i]/curve1[2][i])-curve1[4][i])/curve1[2][i];
        }

    scanf("%d",&n2);
    curve2=RequestMatrixDoubleMem(6,n2);
    for (int i=0;i<n2;i++)
        scanf("%lf %lf",&curve2[0][i],&curve2[1][i]);
        curve2[4][0]=(curve2[1][1]-curve2[1][0])/(curve2[0][1]-curve2[0][0]);
    for (int i=0;i<n2-1;i++){
        curve2[2][i]=curve2[0][i+1]-curve2[0][i];
        curve2[3][i]=curve2[1][i+1]-curve2[1][i];
        curve2[4][i+1]=2*(curve2[3][i]/curve2[2][i])-curve2[4][i];
        curve2[5][i]=((curve2[3][i]/curve2[2][i])-curve2[4][i])/curve2[2][i];
        }

    scanf("%d",&n3);
    curve3=RequestMatrixDoubleMem(6,n3);
    for (int i=0;i<n3;i++)
        scanf("%lf %lf",&curve3[0][i],&curve3[1][i]);
        curve3[4][0]=(curve3[1][1]-curve3[1][0])/(curve3[0][1]-curve3[0][0]);
    for (int i=0;i<n3-1;i++){
        curve3[2][i]=curve3[0][i+1]-curve3[0][i];
        curve3[3][i]=curve3[1][i+1]-curve3[1][i];
        curve3[4][i+1]=2*(curve3[3][i]/curve3[2][i])-curve3[4][i];
        curve3[5][i]=((curve3[3][i]/curve3[2][i])-curve3[4][i])/curve3[2][i];
        }
    ...
    freeMatrizDouble(curve1,6);
    freeMatrizDouble(curve2,6);
    freeMatrizDouble(curve3,6);
	return 0;
}
